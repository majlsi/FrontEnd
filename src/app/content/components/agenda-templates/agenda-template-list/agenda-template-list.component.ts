...
@Component({
	selector: 'm-agenda-template-list',
	templateUrl: './agenda-template-list.component.html',

})


export class AgendaTemplateListComponent implements OnInit, AfterViewInit{
	isCollapsed: boolean = false;
	dataSource: Array<AgendaTemplate> = [];
	...
	displayedColumns = ['agenda_template_name_ar', 'agenda_template_name_en', 'actions'];
	...
	getList() {
		...
		this._crudService.getPaginatedList<PagedResult>('admin/agenda-templates', this.filterObject).
			subscribe(res => {
	...
	edit(id: any) {
		this.router.navigate(['/agenda-templates/edit', id]);
	}

	delete(id: any) {
		const _title: string = this.translate.instant('AGENDA_TEMPLATE.DELETE.DELETEAGENDATEMPLATE');
		const _description: string = this.translate.instant('AGENDA_TEMPLATE.DELETE.DESCRIPTION');
		const _waitDesciption: string = this.translate.instant('AGENDA_TEMPLATE.DELETE.WAITDESCRIPTION');
		const _deleteMessage = this.translate.instant('AGENDA_TEMPLATE.DELETE.DELETEMESSAGE');
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this._crudService.delete<AgendaTemplate>('admin/agenda-templates', id).
				subscribe(pagedData => {
	...
	checkAddFlag() {
		this.roleService.canAccess(Right.AGENDATEMPLATEADD).subscribe(res => {
			if (res.canAccess === 1) {
				this.addFlag = true;
			}
		}, error => { });
	...
	checkEditFlag() {
		this.roleService.canAccess(Right.AGENDATEMPLATEEDIT).subscribe(res => {
			if (res.canAccess === 1) {
				this.editFlag = true;
			}
		}, error => { });
	...
	checkDeleteFlag() {
		this.roleService.canAccess(Right.AGENDATEMPLATEDELETE).subscribe(res => {
			if (res.canAccess === 1) {
