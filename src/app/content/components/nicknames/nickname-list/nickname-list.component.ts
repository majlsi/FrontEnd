...
export class NicknameListComponent implements OnInit {
	...
	displayedColumns = ['nickname_ar', 'nickname_en', 'actions'];
	...
	getList() {
		...
		this._crudService.getPaginatedList<PagedResult>('admin/nicknames', this.filterObject).
			subscribe(res => {
	...
	edit(id: any) {
        this.router.navigate(['/nicknames/edit', id]);
    }

	delete(id: any) {
		const _title: string = this.translate.instant('NICKNAMES.DELETE.DELETENICKNAME');
		const _description: string = this.translate.instant('NICKNAMES.DELETE.DESCRIPTION');
		const _waitDesciption: string = this.translate.instant('NICKNAMES.DELETE.WAITDESCRIPTION');
		const _deleteMessage = this.translate.instant('NICKNAMES.DELETE.DELETEMESSAGE');
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this._crudService.delete<Nickname>('admin/nicknames', id).
            subscribe(pagedData => {
	...
	checkAddFlag(){
		this.roleService.canAccess(Right.ADDNEWNICKNAME).subscribe(res => {
				if (res.canAccess === 1) {
					this.addFlag = true;
				}
			}, error => {});
	...
	checkEditFlag(){
		this.roleService.canAccess(Right.EDITNICKNAME).subscribe(res => {
				if (res.canAccess === 1) {
					this.editFlag = true;
				}
			}, error => {});
	...
	checkDeleteFlag(){
		this.roleService.canAccess(Right.DELETE_NICK_NAME).subscribe(res => {
			if (res.canAccess === 1) {
