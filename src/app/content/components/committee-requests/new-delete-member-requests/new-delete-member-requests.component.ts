...
@Component({
  selector: 'm-new-delete-member-requests',
  templateUrl: './new-delete-member-requests.component.html',
  changeDetection: ChangeDetectionStrategy.Default
})
export class NewDeleteMemberRequestsComponent implements OnInit,AfterViewInit{
	...
	getList() {
	  ...
	  this._crudService.getPaginatedList<PagedResult>('requests/delete-member-committee', this.filterObject).
		subscribe(res => {
	...
	details(id) {
	  this.router.navigate(['/committee-requests/delete-member', id]);
	}

	exportDeleteMembersRequests() {
		this._requestService.exportDeleteMemberFromCommitteesRequestsData().subscribe(
			res => {
					const downloadURL = window.URL.createObjectURL(res);
					const link = document.createElement('a');
					link.href = downloadURL;
					link.download =  this.translate.instant('REQUEST.EXPORT.DELETE_MEMBERS_REQUESTS') + '.xlsx';
					link.click();
			},
			error => {
	  })
