...
@Component({
	selector: "m-delete-committee-member-details",
	templateUrl: "./delete-committee-member-details.component.html",
	styleUrls: ["./delete-committee-member-details.component.scss"],
})
export class DeleteCommitteeMemberDetailsComponent implements OnInit {
	user = { name_ar: "", email: "", job_id: "", job_title: "", Responsible_admininstartation: "", transfer_NO: "", committee_name: "", delete_reason: "" };
	requestId: number;
	request: any;
	userId: number;
	files = [];
	...
	constructor(private _crudService: CrudService, private route: ActivatedRoute, private router: Router, private _translationService: TranslationService, private layoutUtilsService: LayoutUtilsService, private translate: TranslateService, private uploadService: UploadService, private addUserRequestService: AddUserRequestService, private _modalService: NgbModal) {
	...
	getRequest() {
		this._crudService.get<Request>(`requests`, this.requestId).subscribe(
			(data) => {
				...
				this.user.delete_reason = data["request_body"]["delete_reason"];
	...
	accept() {
		this.submitted = true;
		this.addUserRequestService.acceptDeleteMemberRequest(this.requestId).subscribe(
			(data) => {
				this.redirect();
			},
			(error) => {
