...
export class TemplateListComponent implements OnInit, AfterViewInit {
	...
	displayedColumns = ['template_name_ar', 'template_name_en', 'actions'];
	...
	getList() {
		...
		this._crudService.getPaginatedList<PagedResult>('admin/mom-templates', this.filterObject).
			subscribe(res => {
	...
	edit(id: any) {
		this.router.navigate(['/mom-templates/edit', id]);
	}

	delete(id: any) {
		const _title: string = this.translate.instant('MOM_TEMPLATE.DELETE.DELETEMOMTEMPLATE');
		const _description: string = this.translate.instant('MOM_TEMPLATE.DELETE.DESCRIPTION');
		const _waitDesciption: string = this.translate.instant('MOM_TEMPLATE.DELETE.WAITDESCRIPTION');
		const _deleteMessage = this.translate.instant('MOM_TEMPLATE.DELETE.DELETEMESSAGE');
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this._crudService.delete<MomTemplate>('admin/mom-templates', id).
				subscribe(pagedData => {
	...
	checkAddFlag() {
		this.roleService.canAccess(Right.MOMTEMPLATEADD).subscribe(res => {
			if (res.canAccess === 1) {
				this.addFlag = true;
			}
		}, error => { });
	...
	checkEditFlag() {
		this.roleService.canAccess(Right.MOMTEMPLATEEDIT).subscribe(res => {
			if (res.canAccess === 1) {
				this.editFlag = true;
			}
		}, error => { });
	...
	checkDeleteFlag() {
		this.roleService.canAccess(Right.MOMTEMPLATEDELETE).subscribe(res => {
			if (res.canAccess === 1) {
