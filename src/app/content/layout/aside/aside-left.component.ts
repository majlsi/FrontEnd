...
import { RightOrder } from '../../../core/models/enums/rightOrder';
...
export class AsideLeftComponent implements OnInit, AfterViewInit {
	...
	ngOnInit() {
		this._chatService.newChatNotificationsIdsUpdate.subscribe((data) => {
			if (data) {
				this.markUnread = data.length == 0 ? false : true;
			}
		});
	...
	isMenuItemIsActive(item): boolean {
		...
		if (this.currentRouteUrl === '/' && (item.page === 'meeting-dashboard' || item.page === '/meeting-dashboard')) {
			return true;
		}
