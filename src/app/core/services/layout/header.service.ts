...
export class HeaderService {
	...
	nameEn: BehaviorSubject<string> = new BehaviorSubject('');

	nameAr: BehaviorSubject<string> = new BehaviorSubject('');
	...
	setImageURL(imageURL: string) {
		this.profileImage.next(environment.imagesBaseURL + imageURL);
	}

	setNameEn(name: string) {
		this.nameEn.next(name);
	}

	setNameAr(name: string) {
		this.nameAr.next(name);
	}
}
